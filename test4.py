...
def sendEmail(subject, body, error=None):
...
urls = [
    'https://production.techsavanna.technology/fgshr',
    ...
    'https://ippf.techsavanna.technology'
]
...
for url in urls:
    ...
    try:
        subprocess.check_output(['ping', '-c', '1', '-W', '1', domain])
        ...
        try:
            ...
            if os.path.exists(myfile):
                ...
                files_to_delete = [filename for filename in os.listdir() if filename.startswith("index")]
...
if offline_urls:
        subject = 'Application Critical Error report'
        body = "The following applications' URLs are unreachable:\n\n{}".format('\n'.join(offline_urls))
        sendEmail(subject, body)
